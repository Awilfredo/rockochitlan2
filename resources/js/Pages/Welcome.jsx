...
export default function Welcome({
    ...
    const handleEdit = (contentKey, text, id) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('page-contents.update', data.id), {
            preserveScroll: true,
            onSuccess: () => {
                setShowEditContenModal(false);
            },
        });
    };
    ...
    return (
        <DefaultLayout>
            ...
            <div className="flex h-56 w-full flex-wrap sm:h-64 xl:h-80 2xl:h-96">
                ...
                {user?.roles.includes('admin') ? (
                    <div className="mt-5 flex w-full justify-center">
                        <EditButton
                            onClick={() => router.visit(route('images.create'))}
                        ></EditButton>
                    </div>
                ) : (
                    ''
                ...
                <div className="card m-10 max-w-4xl">
                    <div className="flex flex-wrap justify-center gap-4">
                        <h1 className="text-center text-3xl">
                            {pageContents?.find(
                                (content) => content.key === 'home_title',
                            )?.content
                                ? pageContents?.find(
                                      (content) => content.key === 'home_title',
                                  ).content
                    ...
                    <p className="mt-5 text-xl">
                        {pageContents?.find(
                            (content) => content.key === 'home_description',
                        )?.content
                            ? pageContents?.find(
                                  (content) =>
                                      content.key === 'home_description',
                              ).content
                            : 'Ven y vive una gran experiencia con nuestros exquisitos platos y las mejores cervezas acompañado  la buena Música. Trae a toda tu familia y amigos para vivir un momento maravilloso, lleno de sensaciones al paladar.'}
                        {user?.roles.includes('admin') ? (
                            <EditButton
                                className="ml-5"
                                onClick={() =>
            ...
            <EditContent
                show={showEditContenModal}
                onClose={() => setShowEditContenModal(false)}
                data={data}
                setData={setData}
